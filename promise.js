...
const createThreePosts = async ()=>{
    promise1 = createPost({ title: "Post One" , body:"This is post one", createdAt: undefined});
    promise2 = createPost({title:"Post Two", body:"This is post two", createdAt: undefined});
    promise3 = createPost({title:"Post Three", body:"This is Post Three", createdAt:undefined});

    let resolve;
    let finalResult = "";

    resolve = await Promise.all([updateLastUserActiveTime(),promise1]);
    finalResult+=`Created ${resolve[1]} || last active time ${resolve[0]} \n`;
    resolve = await Promise.all([updateLastUserActiveTime(),promise2]);
    finalResult+=`Created ${resolve[1]} || last active time ${resolve[0]} \n`;
    resolve = await Promise.all([updateLastUserActiveTime(),promise3]);
    finalResult+=`Created ${resolve[1]} || last active time ${resolve[0]} \n`;
...
createThreePosts().then((m)=>console.log(m));
